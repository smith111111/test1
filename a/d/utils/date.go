...
func GetFirstDayAndLastDayOfWeek(year, week int) (firstDay, lastDay time.Time) {
	firstDay = time.Date(year, 0, 0, 0, 0, 0, 0, time.Now().Local().Location())
	...
	lastDay = time.Date(firstDay.Year(), firstDay.Month(), firstDay.Day()+6, 23, 59, 59, 0, time.Now().Local().Location())
