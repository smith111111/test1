...
import (
	...
	"gcwallet/multiwallet/cache"
	"gcwallet/multiwallet/boltdb"
	"gcwallet/multiwallet/client"
	"gcwallet/multiwallet/config"
	"gcwallet/multiwallet/keys"
	"gcwallet/multiwallet/service"
	"gcwallet/multiwallet/util"
	"gcwallet/multiwallet/exchangerates"
...
func NewBitcoinWallet(cfg config.CoinConfig, mnemonic string, params *chaincfg.Params, proxy proxy.Dialer, cache cache.Cacher, disableExchangeRates bool) (*BitcoinWallet, error) {
	...
	fp := NewFeeProvider(cfg.MaxFee, cfg.HighFee, cfg.MediumFee, cfg.LowFee, cfg.FeeAPI, proxy)
...
func (w *BitcoinWallet) Transactions(transactionType, sort, offset, limit int) ([]wi.Txn, int32, error) {
	height, _ := w.ChainTip()
	txns, count, err := w.db.Txns().GetAllByLimit(false, transactionType, sort, offset, limit)
	if err != nil {
