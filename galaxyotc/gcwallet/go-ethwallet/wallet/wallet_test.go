...
func setupCoinConfigRopsten() {
	...
	cfg.ClientAPIs = []string{"https://ropsten.infura.io"}
...
func setupCoinConfigRinkeby() {
	...
	cfg.ClientAPIs = []string{"https://rinkeby.infura.io"}
	cfg.CoinType = wi.Ethereum
	cfg.Options = make(map[string]interface{})
...
func TestWalletSpend(t *testing.T) {
	...
	h, err := validSampleWallet.Spend(value.String(), destWallet.address, 1, magicOrderID)
