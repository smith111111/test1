...
func NewERC20Wallet(cfg config.CoinConfig, mnemonic string, proxy proxy.Dialer) (*ERC20Wallet, error) {
	...
	deployAddrMain, ok := cfg.Options["MainNetAddress"]
	if !ok {
	...
	deployAddrRopsten, ok := cfg.Options["RopstenAddress"]
	if ok {
		token.deployAddressMain = common.HexToAddress(deployAddrRopsten.(string))
	}

	deployAddrRinkeby, ok := cfg.Options["RinkebyAddress"]
	if ok {
	...
	ws := NewWalletService(cfg.DB, client.Client, cfg.CoinType, addr, ea, deployAddrMain.(string))
...
func (wallet *ERC20Wallet) BalanceBigInt() (confirmed, unconfirmed big.Int) {
...
func (wallet *ERC20Wallet) Transactions(transactionType, sort, offset, limit int) ([]wi.Txn, int32, error) {
...
func (wallet *ERC20Wallet) Spend(amount string, addr btcutil.Address, feeLevel wi.FeeLevel, referenceID string) (*chainhash.Hash, error) {
	...
	if isScript {
		...
		hash, err = wallet.callAddTokenTransaction(ethScript, amountBig)
	} else {
...
func (wallet *ERC20Wallet) createTxnCallback(txID, orderID string, value string, bTime time.Time) wi.TransactionCallback {
